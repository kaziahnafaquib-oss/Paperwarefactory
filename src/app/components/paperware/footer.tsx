...
export function Footer({ onPageChange }: { onPageChange: (page: string) => void }) {
  ...
  return (
    <footer className="bg-[#fabf37] text-black pt-0 relative overflow-hidden">
      {/* Social Scrolling Ticker */}
      <div className="w-full bg-black py-3 overflow-hidden relative z-20 border-b-4 border-[#fabf37]">
        <Motion.div 
          animate={{ x: ["0%", "-50%"] }}
        ...
        >
          {[...Array(6)].flatMap((_, i) => [
            { text: "LIKE ON FACEBOOK", icon: Facebook },
            { text: "CONNECT ON LINKEDIN", icon: Linkedin },
            { text: "FOLLOW ON INSTAGRAM", icon: Instagram }
          ].map((item, idx) => (
            <div key={`${i}-${idx}`} className="flex items-center gap-4">
              <item.icon className="size-4 text-[#fabf37]" />
              <span className="text-[#fabf37] font-black uppercase tracking-[0.2em] text-xs">{item.text}</span>
              <div className="size-3">
                <svg viewBox="0 0 24 24" fill="none" className="w-full h-full text-zinc-800">
